...
import type { Snapshot, Session, AppConfig } from "./types";
...
function mockApi(): Api {
  ...
  let sessions: Session[] = [
    { id: "api", folder: "api-gateway", cwd: "~/dev/api-gateway", container: true, state: "waiting", lastMsg: "Uso NSIS ou MSI pro instalador do Windows?", updatedAt: now - 2_000 },
    { id: "sem", folder: "claude-semaforo", cwd: "~/dev/claude-semaforo", container: false, state: "waiting", lastMsg: "Mantenho compatibilidade com a v1?", updatedAt: now - 60_000 },
    { id: "mob", folder: "mobile-app", cwd: "~/work/mobile-app", container: true, state: "working", lastMsg: "Refatorando o módulo de autenticação…", updatedAt: now - 40_000 },
    { id: "mkt", folder: "marketing-site", cwd: "~/dev/marketing-site", container: false, state: "ready", lastMsg: "Refiz a hero e os três cards de preço.", updatedAt: now - 180_000 },
    { id: "dat", folder: "data-pipeline", cwd: "~/dev/data-pipeline", container: false, state: "ready", lastMsg: "Tudo verde: 42/42 testes passando.", updatedAt: now - 360_000 },
  ];

  const ASK = ["Uso NSIS ou MSI pro instalador do Windows?", "Quer dark mode também?", "Mantenho compatibilidade com a v1?", "Qual nome dou pro endpoint novo?", "Componente separado ou inline?"];
  ...
  const doWait = () => { const s = pick("working") || pick("ready") || pickNot("waiting"); if (s) patch(s.id, { state: "waiting", lastMsg: rnd(ASK) }); };
  const doFinish = () => { const s = pick("working") || pickNot("ready"); if (s) patch(s.id, { state: "ready", lastMsg: rnd(DONE) }); };
  const doPrompt = () => { const s = pick("waiting") || pick("ready") || pickNot("working"); if (s) patch(s.id, { state: "working", lastMsg: rnd(WORK) }); };

  const tick = () => {
    const w = sessions.filter((x) => x.state === "waiting").length;
    const k = sessions.filter((x) => x.state === "working").length;
    const r = sessions.filter((x) => x.state === "ready").length;
    if (w >= 2) return doPrompt();
    if (k === 0 && w === 0) return doPrompt();
    const d = Math.random();
    if (d < 0.35 && k > 0) return doWait();
    if (d < 0.75 && k > 0) return doFinish();
    if (w > 0) return doPrompt();
